  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (isRateLimit && attempt < retries) {
        ...
        console.warn(`Notion rate limited. Retrying in ${delay}ms (attempt ${attempt + 1}/${retries})...`);
        await sleep(delay);
        continue;
      }
      throw err;
...
export async function getPublishedProjects() {
  ...
  while (hasMore) {
    ...
    const response = await withRetry(() => notion.databases.query(queryOptions));
...
export async function getAllPostsSorted() {
  ...
  const allPosts = [
    ...
    ...websites.map(p => ({ ...p, category: "websites" })),
    ...writing.map(p => ({ ...p, category: "writing" })),
    ...books.map(p => ({ ...p, category: "books" })),
    ...florals.map(p => ({ ...p, category: "florals" })),
  ];
  ...
  // Filter only those with slugs (except florals which are currently gallery only)
  // and sort chronologically (newest first)
  return allPosts
    .filter(p => !!p.Slug || p.category === "florals")
